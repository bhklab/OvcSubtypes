...
<<setup_robustness, include=FALSE>>=
# Set options
rm(list=ls())
...
if(file.exists("konecny.eset.RData") && file.exists("konecny.original.eset.RData")) {
	load("konecny.eset.RData")
  load("konecny.original.eset.RData")
} else {
...
save(tothill.merged.kmeans, file="tothill.merged.kmeans.RData")
