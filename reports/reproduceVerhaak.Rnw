...
implemented.verhaak.output <- lapply(verhaak.validation.esets, get.subtypes, method="Verhaak")
@
...
for(i in 1:length(implemented.verhaak.output)) {
  ...
  if(Dataset.name == "Crijns") {
    crijns.patientdata <- read.xls("docs/Patient data + GEO coupling for Haibe-Kains.xls")
  ...
  implemented.ssgsea.scores <- rbind(implemented.ssgsea.scores, data.frame(
                                                                      ...
                                                                      implemented.subtype=implemented.verhaak.output[[i]]$Verhaak.subtypes,
                                                                      days_to_death=verhaak.validation.esets[[i]]$days_to_death,
                                                                      vital_status=verhaak.validation.esets[[i]]$vital_status))
}
...
km.coxph.plot(surv.obj ~ implemented.subtype, verhaak.merged, x.label="Time (months)", y.label = "Survival", main.title="Survival: Implemented Classifier", show.n.risk = FALSE, leg.text = levels(verhaak.merged$implemented.subtype), .col=c("green", "#E38010", "purple", "blue"), leg.pos="topright", leg.inset=0, n.risk.cex=0.85, cex=0.4)
@
\includegraphics[width=0.5\textwidth]{images/verhaak_fig2B.png}
