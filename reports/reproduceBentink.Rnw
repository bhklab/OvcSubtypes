...
crijns.eset <- esets$GSE13876
tothill.eset <- esets$GSE9891
dressman.eset <- esets$PMID17290060
bentink.eset <- esets$E.MTAB.386
denkert.eset <- esets$GSE14764
mok.eset <- esets$GSE18520
...
implemented.bentink.out <- lapply(implemented.bentink.esets, function(x) {
  x <- x[,!is.na(x$histological_type) & !is.na(x$summarygrade) & !is.na(x$summarystage) & x$histological_type=="ser" & x$summarygrade=="high" & x$summarystage=="late"]
  x <- x[,!is.na(x$days_to_death) & !is.na(x$vital_status)]
})

implemented.bentink.out <- lapply(implemented.bentink.esets, get.subtypes, method="Bentink")

bentink.plots <- lapply(names(implemented.bentink.out), function(dataset.name) {
  data.to.plot <- pData(implemented.bentink.esets[[dataset.name]])
  data.to.plot$Bentink.subtypes <- implemented.bentink.out[[dataset.name]]$Bentink.subtypes
...
\includegraphics[width=\textwidth]{images/bentink_supp2_tothill.png}
...
\includegraphics[width=\textwidth]{images/bentink_supp2_crijns.png}
...
\includegraphics[width=\textwidth]{images/bentink_supp2_denkert.png}
...
\includegraphics[width=\textwidth]{images/bentink_supp2_dressman.png}
...
\includegraphics[width=\textwidth]{images/bentink_supp2_mok.png}
