...
We sought to develop an subtype classifier that represents the consensus of published HGS ovarian cancer subtype classifiers. We sought to design this classifier to be trained on multiple datasets, applicable across platforms, and able to discriminate between patients that are of well-defined or intermediate subtypes. 

To this end, we present two consensus classifiers. The first is a random forest classifier, trained on concordantly classified patients across our ovarian cancer data compendium. This classifier uses real-valued gene expression across platforms to classify patients in independent datasets; it outputs a classification, in addition to a real-valued margin score which can discriminate between patients that are of well-defined or intermediate subtypes. The second consensus classifier is based on binary gene pairs, and was designed to be appropriate for classifying expression profiles in small (e.g. single-sample) independent datasets.
...
plot(ROCR::performance(ROCR::prediction(pooled.subtypes$Consensus.genepairs.rf.margin[margin.comparison.cases.to.include], pooled.subtypes$concordant[margin.comparison.cases.to.include]), "tpr", "fpr"), col=roc.colors[5], lwd=2, add=TRUE)
