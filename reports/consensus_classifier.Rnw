...
for(classifier in c("Gene Pairs RF", "Gene Pairs RF (default cutoff)")) {
  ...
  #par(mar=c(5.1, 5, 4.1, 2.1))
    ...
    } else if (classifier == "Gene Pairs RF (default cutoff)") {
...
#    text(0,0.05, text, cex=0.85, pos=4)
@
\caption{Survival curves for patients across our ovarian cancer data compendium. Classification was performed using leave-one-dataset-out validation. For the bottom two figures, classification with the consensus classifier was performed with the default cutoff, in which 0.75 of patients with the lowest margin are not classified.}
\end{figure}
