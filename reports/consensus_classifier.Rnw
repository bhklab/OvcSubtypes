...
ggplot(roc_Helland.df, aes(x =x, y = y)) +
    ...
    theme_bw() + xlab("False positive rate") + ylab("True positive rate") + ylim(0, 1) + theme(legend.position = c(0.75, 0.25)) +  labs(color="") + scale_colour_manual("", values=c("red", "gold", "orange", "orchid3")) 
...
<<Figure5-MarginAnalysis, fig.keep ='last', fig.width=10, fig.height=11, out.width="0.7\\textwidth", cache=FALSE>>=
...
grid.arrange(
Pane_5A, Pane_5B, layout_matrix = matrix(c(1,2), nrow = 2, byrow = TRUE), heights = c(20,30))
