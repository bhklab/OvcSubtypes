...
implemented.helland.out <- get.subtypes(tcga.eset, method="Helland")
tcga.eset$Helland.subtypes <- implemented.helland.out$Helland.subtypes
# Labelled validation data, provided by the authors by email
helland.labelled.validation <- read.csv("docs/TCGA_samples_classified.csv")
...
helland.merged <- merge(pData(tcga.eset)[,c("unique_patient_ID", "vital_status", "days_to_death", "Helland.subtypes")], helland.labelled.validation, by="unique_patient_ID")
...
helland.data.for.survival <- pData(tcga.eset)[,c("unique_patient_ID", "days_to_death", "vital_status", "Helland.subtypes")]
...
km.coxph.plot(surv.obj ~ Helland.subtypes, helland.data.for.survival, x.label="Time (months)", y.label = "Survival", main.title="Survival: Implemented Helland classifier", show.n.risk = FALSE, leg.text = levels(helland.data.for.survival$Helland.subtypes), .col=c("red", "green", "blue", "black"), leg.pos="topright", leg.inset=0, n.risk.cex=0.85, cex=0)
@

\includegraphics[width=0.5\textwidth]{images/helland_fig1B_TCGA.png}
