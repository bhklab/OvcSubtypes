...
# in progress
tcga.ps.vals <- sapply(names(classifier.subtypes$Verhaak), function(dataset.name) {
  median.clustering.index <- which(tcga.ps.vals[,dataset.name] == median(tcga.ps.vals[,dataset.name]))[1]
  sapply(1:100, function(y) ps.cluster(classifier.subtypes$Verhaak[[x]], clustering.subtypes$tcga_nmf_4[[x]][[y]])$ps))
})
@
