...
tcga.pred.str.trend <- lapply(names(classifier.subtypes$Verhaak), function(dataset.name) {
  ...
  xvals <- seq(0, length(classifier.subtypes$Verhaak[[dataset.name]]), by = 25)[-1:-2]
...
tothill.pred.str.trend <- lapply(names(classifier.subtypes$Helland), function(dataset.name) {
  ...
  xvals <- seq(0, length(classifier.subtypes$Helland[[dataset.name]]), by=25)[-1:-2]
  ps.vals <- sapply(xvals, function(x) {
    # calculate Prediction Strength; use the top x samples (by margin)
...
tothill.pred.str.trend.tcga.ps.vals <- sapply(tothill.pred.str.trend.tcga.xvals, function(x) {
    ...
    #samples.to.keep <- margin.vals >= sort(margin.vals)[length(margin.vals)-x+1]
    current.ps <- ps.cluster(classifier.subtypes$Helland$TCGA[samples.to.keep], clustering.subtypes$tothill_kmeans_4$TCGA[[median.clustering.index]][samples.to.keep])$ps
    return(current.ps)
  })
...
konecny.pred.str.trend <- lapply(names(classifier.subtypes$Konecny), function(dataset.name) {
  ...
  xvals <- seq(0, length(classifier.subtypes$Konecny[[dataset.name]]), by=25)[-1:-2]
...
konecny.pred.str.trend.tcga.ps.vals <- sapply(konecny.pred.str.trend.tcga.xvals, function(x) {
    ...
    samples.to.keep <- margin.vals > quantile(margin.vals, x)
    #samples.to.keep <- margin.vals >= sort(margin.vals)[length(margin.vals)-x+1]
    current.ps <- ps.cluster(classifier.subtypes$Konecny$TCGA[samples.to.keep], clustering.subtypes$konecny_nmf_4$TCGA[[median.clustering.index]][samples.to.keep])$ps
    return(current.ps)
  })
...
@
%Note that Prediction Strength for the TCGA dataset drops to zero between the dataset sizes of 58 and 68. This is an artifact in the Prediction Strength statistic that occurs when the number of samples in a class is very small; for lower than 58 patients, only three subtypes are produced from both clustering and classification. 
