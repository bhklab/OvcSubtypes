...
for(dirname in c("konecny_nmf_4", "tcga_nmf_4", "tothill_kmeans_4")) {
  ...
  for(current.eset.name in dataset.names) {
    ...
    filenames <- Sys.glob(paste0("jan20clusters/", dirname, "/", current.eset.name, "_*_classes.txt"))
...
tcga.ps.vals <- sapply(1:length(classifier.subtypes$Verhaak), function(x) sapply(1:42, function(y) ps.cluster(classifier.subtypes$Verhaak[[x]], clustering.subtypes$tcga_nmf_4[[x]][[y]])$ps))
...
tothill.ps.vals <- sapply(1:length(classifier.subtypes$Helland), function(x) sapply(1:42, function(y) ps.cluster(classifier.subtypes$Helland[[x]], clustering.subtypes$tothill_kmeans_4[[x]][[y]])$ps))
...
konecny.ps.vals <- sapply(1:length(classifier.subtypes$Konecny), function(x) sapply(1:42, function(y) ps.cluster(classifier.subtypes$Konecny[[x]], clustering.subtypes$konecny_nmf_4[[x]][[y]])$ps))
...
For each classification and clustering algorithm, we produced a stripplot of the prediction strength values for each of the 100 clustering runs. In order to investigate a subtype-specific measure of robustness, we plot the subtype-specific Rand Index for each subtype. For a given subtype (as defined by classifier labels), the subtype-specific Rand Index is defined by the fraction of pairs that co-cluster divided by the total number of pairwise relationships.
...
<<tcga_adj_rand, out.width="0.6\\textwidth">>=
...
<<tothill_adj_rand, out.width="0.6\\textwidth">>=
...
<<konecny_adj_rand, out.width="0.6\\textwidth">>=
