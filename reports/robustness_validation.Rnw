...
tcga.pred.str.trend <- lapply(names(classifier.subtypes$Verhaak), function(dataset.name) {
  ...
  ps.vals <- sapply(50:length(classifier.subtypes$Verhaak[[dataset.name]]), function(x) {
  ...
  out.df <- data.frame(dataset.size=50:length(classifier.subtypes$Verhaak[[dataset.name]]), prediction.strength=ps.vals)
  return(out.df)
})
...
tothill.pred.str.trend <- lapply(names(classifier.subtypes$Helland), function(dataset.name) {
  ...
  ps.vals <- sapply(50:length(classifier.subtypes$Helland[[dataset.name]]), function(x) {
    # calculate Prediction Strength; use the top x samples (by margin)
  ...
  out.df <- data.frame(dataset.size=50:length(classifier.subtypes$Helland[[dataset.name]]), prediction.strength=ps.vals)
  return(out.df)
})
...
konecny.pred.str.trend <- lapply(names(classifier.subtypes$Konecny), function(dataset.name) {
  ...
  ps.vals <- sapply(50:length(classifier.subtypes$Konecny[[dataset.name]]), function(x) {
    # calculate Prediction Strength; use the top x samples (by margin)
  ...
  out.df <- data.frame(dataset.size=50:length(classifier.subtypes$Konecny[[dataset.name]]), prediction.strength=ps.vals)
  return(out.df)
})
...
<<dataset_barplot, fig.width=8, fig.height=5, eval=FALSE>>=
...
\pagebreak
Next, we computed Prediction Strength using various dataset sizes. For each classifier, we included validation datasets with greater than 100 late-stage high-grade serous patients. We excluded the dataset used to train the original classifier, and restricted margin cutoff values to include at least 50 patients. Using classifier margin values to define cutoffs, we computed Prediction Strength for subsets of each dataset.
\subsection{TCGA/Verhaak}
<<tcga_pred_str_margin_trend>>=
ggplot(tcga.pred.str.trend.combined, aes(y=prediction.strength, x = dataset.size, colour = dataset.name)) + geom_point(size=1) + theme_bw() + xlab("Dataset Size") + ylab("Prediction Strength: TCGA/Verhaak") + ylim(0, 1) + ggtitle("Prediction Strength") + scale_x_reverse()
@
\pagebreak
...
ggplot(tothill.pred.str.trend.combined, aes(y=prediction.strength, x = dataset.size, colour = dataset.name)) + geom_point(size=1) + theme_bw() + xlab("Dataset Size") + ylab("Prediction Strength: Tothill/Helland") + ylim(0, 1) + ggtitle("Prediction Strength") + scale_x_reverse()
@
\pagebreak
\subsection{Konecny}
...
ggplot(konecny.pred.str.trend.combined, aes(y=prediction.strength, x = dataset.size, colour = dataset.name)) + geom_point(size=1) + theme_bw() + xlab("Dataset Size") + ylab("Prediction Strength: Konecny") + ylim(0, 1) + ggtitle("Prediction Strength") + scale_x_reverse()
@
