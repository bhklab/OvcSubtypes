...
out.dir <- paste0("may2clusters_concordant/", gene.set, "_", algorithm, "_", k)
dir.create("may2clusters_concordant/")
dir.create(out.dir)

current.eset <- esets.not.rescaled.classified.concordant[[dataset.index]]
current.eset.name <- names(esets.not.rescaled.classified.concordant)[dataset.index]
