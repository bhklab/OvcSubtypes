...
margin.scatterplots.data <- function(subtype.name.pair) {
    margins.1 <- pooled.subtypes[,paste0(subtype.name.pair[1], ".margins")]
    margins.2 <- pooled.subtypes[,paste0(subtype.name.pair[2], ".margins")]  
    margins.1 <- ecdf(margins.1)(margins.1)
    margins.2 <- ecdf(margins.2)(margins.2) 
...
margin.scatterplots <- apply(pair.matrix[,c(1,3,5)], 2, function(subtype.name.pair) {   
    ...
    scatterplot <- ggplot(scatterplot.df, aes(x=margin, y=concordance)) + geom_point(size=0) + geom_line(size=0) + theme_bw() + xlab("% of dataset removed") + ylab("Concordance") + ggtitle(paste0(subtype.name.pair[1], " vs ", subtype.name.pair[2])) + ylim(0, 1)
    return(scatterplot)
  })
@

<<margin_scatterplots_combined, fig.keep = 'none'>>=
margin.scatterplots.combined <- 
  pairwise_concordances<- apply(pair.matrix[,c(1,3,5)], 2, margin.scatterplots.data)

  scatterplot.df <- data.frame(percent=xvals * 100, 
                               Helland_vs_Verhaak =pairwise_concordances[,1], 
                               Verhaak_vs_Konecny =pairwise_concordances[,2], 
                               Konecny_vs_Helland =pairwise_concordances[,3]
                               )
    scatterplot_melt.df <- melt(scatterplot.df, id = "percent") 
    scatterplot_melt.df <- scatterplot_melt.df[!is.na(scatterplot_melt.df$value),]
    legend_title <- ""
    scatterplots.combined <- ggplot(scatterplot_melt.df, aes(y=value,x=percent, colour=variable)) + scale_colour_manual(legend_title, values=c("gold", "orange", "orchid3")) + geom_line() + theme_bw() + xlab("% of dataset removed") + ylab("Concordance") + ggtitle("Two-way Concordances") + ylim(0, 1) + theme(legend.position = c(0.5, 0.5))
@
...
margin.concordance <- function (subtype_order = 1, all = FALSE) {   
  ...
  legend_title <- ""
  scatterplot_melt.df <- melt(scatterplot.df, id = "percent")       
    subtype_scatterplots <- ggplot(scatterplot_melt.df, aes(y=value, x = percent, colour=variable)) + scale_colour_manual(legend_title, values=c("violet", "red","green","blue", "black")) + geom_line() + theme_bw() + xlab("% of dataset removed") + ylab("Concordance") + ylim(0, 1) + ggtitle("Three-way Concordance") + theme(legend.position = c(0.75, 0.25))
@
...
>>>>>>> 68b5bf2d4e51b7a942ffd1a1cb6a706af8f015a5
...
<<<<<<< HEAD
\begin{figure}[H]
<<Subtype_venn_diagrams, fig.keep ='last', fig.width=4, fig.height=8>>=
=======
...
% % 
% % 
% % vals_margin <- c(get_overlaps_margin[[margin]][[subtype_order]]$lengths[1:6],100(2%)) 
% % names(vals_margin) <- c("hi", "110", "101", "011", "100", "010", "001") 
...
% % plotVenn(vals_margin, names(vals_margin), Colors=cols)
% % plotVenn(vals_margin_overlay, names(vals_margin_overlay), Colors=cols)
% % 
% % # not working
...
=======
<<hr_plots, out.width="0.8\\textwidth", eval=FALSE>>=
...
get_survival_curve <- function (subtype.order) {
  ...
  ggsurv(ss) + theme_bw() + ggtitle(subtype.name)
...
# par(mar=c(5.1, 8, 4.1, 2.1))
# get_survival_curve(1)
# get_survival_curve(2)
# get_survival_curve(3)
# Pane_2_B
# }
