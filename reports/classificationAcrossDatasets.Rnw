...
margin.concordance <- function (subtype_order = 1, all = FALSE) {   
  ...
  scatterplot_melt.df <- melt(scatterplot.df, id = "percent")       
    scatterplot <- ggplot(scatterplot_melt.df, aes(y=value, x = percent, colour=variable)) + scale_colour_manual(values=c("violet", "red","green","blue", "black")) + geom_line() + theme_bw() + xlab("Percentage of dataset removed") + ylab("Concordance") + ylim(0, 1) + ggtitle("Concordance of subtypes") 
@
In the concordance curves of subtypes (above), we can see how the concordance rises for each subtype as the marginal datasets are removed. Each subtype is labelled by the name provied by Verhaak et al.

We filter the dataset based on the percent of datasets that would remain and we remove datasets based on margin-cutoffs (i.e., datasets with margins lesser than the margin cut-offs or removed). In the following plot, we can see the relationship between the margin cutoffs and the percent of dataset removed
