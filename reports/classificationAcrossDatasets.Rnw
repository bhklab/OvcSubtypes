...
\begin{figure}[H]
<<Figure1-ConsensusAnalysis, fig.keep ='last', fig.width=10, fig.height=10>>=
...
% Figure 2
<<Figure2-SurvivalAnalysis, fig.keep ='last', fig.width=10, fig.height=10>>=
...
get_survival_curve <- function (subtype.order) {
  ...
  ggsurv(ss, plot.cens = FALSE, main = subtype.name, back.white = TRUE) + theme(legend.direction='horizontal',legend.box='horizontal',legend.position = c(1, 1), legend.justification = c(1, 1), legend.title=element_blank(), legend.background = element_rect(fill="transparent"))  + xlab("Time (years)") + annotation_custom(grob)
