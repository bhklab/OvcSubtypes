...
subtype.map$"Bentink" <- c(levels(pooled.subtypes.survival[,subtype.names[[4]]]), "Angiogenic", "Angiogenic")
...
margins.1.percentile <- ecdf(margins.1)(margins.1) 

margins.2 <- pooled.subtypes.survival[,paste0(subtype.names[[2]], ".margins")]    
margins.2.percentile <- ecdf(margins.2)(margins.2) 

margins.3 <- pooled.subtypes.survival[,paste0(subtype.names[[3]], ".margins")]    
margins.3.percentile <- ecdf(margins.3)(margins.3) 

# get marginal data 
get_margin <- function(lower.bound){  
    margins.1.percentile > lower.bound & 
    margins.2.percentile > lower.bound & 
    margins.3.percentile > lower.bound
  }
...
get_marginal_data_top <- function(top_margin_percent,subtype.name) {
    ...
    pooled.subtypes.survival[,subtype.name][margin.boolean]
  }
...
get_patients <- function(pos_subtype.name,subtype_order,margin_percent){
  ...
  which(filtered_dataset == subtypes$subtype.name[[subtype_order]] )  
}

# Given the mapped subtype number (called subtype_order), fetch the patients in each scheme in the subtype 
get_subtype_patients <- function(subtype_order,margin_percent){
 lapply(1:3, function(i) {get_patients(i,subtype_order,margin_percent)}) 
}

# Get the subtype_patients for each subtype across schemes and find overlaps 

get_overlaps <- function(margin_percent) {
  lapply(1:4, function(subtype_order) {
    output <- NULL
    output$parts <- calculate.overlap(x=get_subtype_patients(subtype_order,margin_percent))
...
# Concordance plots
margin_vals <- seq(100, 0, length.out = 3001)
get_overlaps_margin <- 
  lapply(margin_vals, function(mval) {get_overlaps(mval)})
  


percent_removed <- lapply(1:3001, function(i) {
  total_remaining = 
...
margin.scatterplots_subtype <- function (subtype_order) {   
  concordance <- lapply(1:3001, function(i) {
...
margin_cutoff = margin_scatterplot.df$margin[1501]
...
We choose a cutoff of \Sexpr{sprintf("%.2f", margin_cutoff * 100)}\% (corresponding to \Sexpr{ sprintf("%.2f", margin_scatterplot.df$percent[margin_scatterplot.df$margin == margin_cutoff])}\% of the dataset removed). The resulting dataset has high subtype concordance as shown in the subtype concordance curves.
...
margin_overlay_cutoff = ceiling(margin_cutoff*3001) %>% as.integer
draw_venn_formargin(1,margin_overlay_cutoff) # since there are 3001 x values; 1 being for all datasets
...
<<Pooled_survival_plot, fig.width=8, fig.height=12>>=
...
# since we name the subtypes of the filtered set to that of Verhaak
  ...
  title <- "Filtered Dataset (before overlaps)"
...
get_hr <- function (margin) {
  ...
  fit = summary(coxph.model)
  # extract the first column which corresponds to the HR of the three subtypes
  fit$conf.int[,1]  
}
...
@
The below plots show that the hazard ratios are more or less the same for filtered data with pretty much all the margins 0 (corresponding to the whole dataset) to 0.6, with our chossen cutoff lying inside this interval. 
