...
xvals <- seq(0, 1, length.out = 3000)
margin.scatterplots_subtype <- function (subtype_order = 1, all = FALSE) {   
  concordance <- 
    if (all) 
    {
  ...
  scatterplot.df <- scatterplot.df[!is.na(scatterplot.df$concordance),]
  title <- 
    if (all) "Overall concordance" else paste0("Concordance of subtype ", subtypes[[2]][[subtype_order]])
    
  scatterplot <- 
    ggplot(scatterplot.df, aes(y=concordance, x = percent)) + geom_point(size=3) + geom_line(size=3) + theme_bw() + xlab("Percentage of dataset removed") + ylab("Concordance") + ylim(0, 1) + ggtitle(title) 
  return(scatterplot)
}
@
...
margin.scatterplots_subtype(all = TRUE)
@
