...
# Need to fix this - currently runs with warnings, but not correct
...
methods = 1:3
margin_vals <- seq(100, 0, length.out = 3000)
find_proportion <- function(subtype.method, subtype_order, margin_percent, compare_self = TRUE){

  if (compare_self) 
   get_overlaps(margin_percent)[[subtype_order]]$lengths[[1]]/length(get_subtype_patients(subtype_order,margin_percent, subtype.method) %>% unlist)
  else
   get_overlaps(margin_percent)[[subtype_order]]$lengths[[1]]/get_overlaps(margin_percent,methods[!methods==subtype.method])[[subtype_order]]$lengths[[3]] 
  
}

get_concordance <- function(subtype.method,subtype_order,compare_self){
lapply(margin_vals[1:1500],
       function (m_v) find_proportion(subtype.method,subtype_order, m_v, compare_self)) %>% unlist
}

get_concordance_all_subtypes <- function(subtype.method, compare_self)
  lapply(1:4, function (i) get_concordance(subtype.method,i, compare_self))
...
get_method_concordance <- function (subtype.method, compare_self = TRUE) {
sc = get_concordance_all_subtypes(subtype.method,compare_self)
 smc.df <- data.frame( percent = percent[1:1500], IMR = sc[[1]], DIF= sc[[2]], PRO = sc[[3]], MES = sc[[4]])
  smc_long.df <- melt(smc.df, id = "percent")                      
                        
txt <- if (compare_self) "within the method" else "under the other two methods"
...
grid.arrange(grobs = lapply(1:3,function (i) get_method_concordance (i, compare_self = FALSE)), ncol = 3, widths=c(6,6,6), heights=c(6,6,6))
@
